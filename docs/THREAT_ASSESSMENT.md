...
- Logical network-reachable protocol/port services, excluding multicast-only discovery memberships; best-effort local process names (never PIDs or command lines); and runtime/permanent firewall drift. Raw IPv4/IPv6/interface bindings are shown separately so they do not inflate exposure findings.
