...
- **Local evidence and credential tampering.** API credentials, operating mode, and network-history records are stored owner-only. The application rejects symlinks and non-regular files for these paths, bounds local reads, and normalizes history fields so untrusted SSID or provider data cannot inject extra TSV rows.
