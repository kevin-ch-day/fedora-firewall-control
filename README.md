...
The default home view is compact for an 80×24 terminal; press `?` to toggle the expanded evidence dashboard. `./build/ffc --snapshot-json` prints the same transport-neutral snapshot as schema-versioned `ffc.dashboard.v1` JSON. It is read-only, opens no network listener, and is the integration boundary for a future local dashboard. A schema-valid snapshot exits `0` even when evidence is partial or unavailable; the assessed risk and each evidence state are contained in JSON, while exit `2` is reserved for a future inability to generate a valid snapshot. The v1 contract is documented in [schemas/dashboard-v1.schema.json](schemas/dashboard-v1.schema.json). It includes the local hostname, active interface/zone, and detected tunnel interface when available, but excludes API keys, credentials, raw command output, journal contents, and saved network-history records.
