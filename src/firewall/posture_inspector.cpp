...
FirewallState DefensivePostureCollector::inspect(const PostureCollectionDepth depth) const {
    ...
    if (depth == PostureCollectionDepth::Complete && !state.network_manager.available) state.errors.push_back("NetworkManager status unavailable: " + state.network_manager.diagnostic);
