...
std::optional<OperatingMode> parse_operating_mode(const std::string_view value) {
    ...
    if (value == "hostile") return OperatingMode::HostileNetwork;
    return std::nullopt;
}
OperatingMode OperatingModeStore::load() const {
    std::string error; const auto path = secure_local_path(LocalStorageArea::Config, "mode", false, error); std::string value;
