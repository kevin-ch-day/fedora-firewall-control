...
bool is_multicast_endpoint(const std::string& endpoint) {
    ...
    if (address.starts_with('[')) {
        ...
        return address.size() >= 2 && std::tolower(static_cast<unsigned char>(address[0])) == 'f' && std::tolower(static_cast<unsigned char>(address[1])) == 'f';
    }
    const auto dot = address.find('.');
    const auto first_octet = address.substr(0, dot);
    unsigned int parsed{};
    const auto [position, error] = std::from_chars(first_octet.data(), first_octet.data() + first_octet.size(), parsed);
    return error == std::errc{} && position == first_octet.data() + first_octet.size() && parsed >= 224U && parsed <= 239U;
}
...
std::string process_name(const std::string& line) {
    ...
    return name_end == std::string::npos ? std::string{} : line.substr(name_start, name_end - name_start);
}
}
...
std::vector<ListeningSocket> parse_listening_sockets(const std::string& ss_output) {
    ...
    while (std::getline(lines, line)) {
        ...
        listeners.push_back({protocol, endpoint, is_loopback_endpoint(endpoint), is_multicast_endpoint(endpoint), process_name(line)});
    }
    return listeners;
}

bool is_network_reachable(const ListeningSocket& listener) { return !listener.loopback_only && !listener.multicast_only; }
...
ListenerExposureSummary summarize_listener_exposure(const SocketState& state) {
    ...
    for (const auto& listener : state.listeners) {
        ...
        if (listener.multicast_only) { ++summary.multicast_only_bindings; continue; }
        ++summary.network_reachable_bindings;
        if (!listener.process_name.empty()) ++summary.process_attributed_network_bindings;
        const auto port = endpoint_port(listener.endpoint);
        if (!port.empty()) services.insert(listener.protocol + "/" + port);
    }
...
SocketState SocketInspector::inspect() const {
    ...
    SocketState state;
    if (!result.success()) { state.diagnostic = result.stderr_text.empty() ? "ss listener query failed" : result.stderr_text; return state; }
