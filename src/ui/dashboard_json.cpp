...
using dashboard_json_detail::escape;
...
std::string json_observation(const ObservationStatus status) {
    return "\"" + std::string{evidence_status(status)} + "\"";
}
...
std::string json_string(const std::string &value, const ObservationStatus status) {
    return status == ObservationStatus::Available ? "\"" + escape(value) + "\"" : "null";
}
...
std::string serialize_dashboard_snapshot_json(const DashboardSnapshot &snapshot) {
    ...
    output
        ...
        << "  \"hostname\": \"" << escape(snapshot.hostname) << "\",\n"
        << "  \"application_version\": \"" << escape(snapshot.application_version) << "\",\n"
        ...
        << "  \"assessment\": {\"mode\": \""
        << (snapshot.firewall.operating_mode == OperatingMode::HostileNetwork ? "hostile"
                                                                              : "normal")
        << "\", \"defcon_readiness\": \"" << readiness(snapshot.defcon_readiness) << "\"},\n"
        << "  \"risk\": {\"level\": \"" << risk_level(snapshot.risk)
        << "\", \"blockers\": " << snapshot.blockers.size()
        << ", \"review_items\": " << snapshot.review_items.size()
        << ", \"coverage_gaps\": " << snapshot.coverage_gaps.size() << "},\n"
        << "  \"firewall\": {\"active_status\": "
        << json_observation(snapshot.firewall.service_state)
        ...
        << ", \"enabled\": "
        << json_bool(snapshot.firewall.enabled, snapshot.firewall.service_enablement)
        << ", \"default_zone_status\": " << json_observation(snapshot.firewall.default_zone_status)
        << ", \"default_zone\": "
        << json_string(snapshot.firewall.default_zone, snapshot.firewall.default_zone_status)
        ...
        << ", \"intra_zone_forwarding\": "
        << json_bool(forwarding, snapshot.firewall.runtime_zones_status) << "},\n"
        << "  \"listeners\": {\"status\": "
        << json_observation(snapshot.firewall.sockets.available ? ObservationStatus::Available
                                                                : ObservationStatus::Unavailable)
        << ", \"tcp_udp_listeners\": "
        << json_count(listeners.logical_network_services, snapshot.firewall.sockets.available
                                                              ? ObservationStatus::Available
                                                              : ObservationStatus::Unavailable)
        << ", \"reachable_bindings\": "
        << json_count(listeners.network_reachable_bindings, snapshot.firewall.sockets.available
                                                                ? ObservationStatus::Available
                                                                : ObservationStatus::Unavailable)
        << "},\n"
        << "  \"network\": {\"physical_interface\": "
        << json_string(active_interface(snapshot.firewall), snapshot.firewall.active_zones_status)
        << ", \"zone\": "
        << json_string(active_zone(snapshot.firewall), snapshot.firewall.active_zones_status)
        << ", \"network_manager\": {\"device_inventory_status\": "
        << json_observation(snapshot.firewall.network_manager.available
                                ? ObservationStatus::Available
                                : ObservationStatus::Unavailable)
        << ", \"profile_inventory_status\": \"unavailable\", \"profile\": null, \"autoconnect\": "
           "null, \"diagnostic\": \""
        << escape(snapshot.firewall.network_manager.available
                      ? "profile and autoconnect inventory are not collected by this read-only "
                        "snapshot"
                      : snapshot.firewall.network_manager.diagnostic)
        << "\"}"
        << ", \"tunnel_detection\": {\"status\": "
        << json_observation(snapshot.firewall.vpn.interface_scan_available
                                ? ObservationStatus::Available
                                : ObservationStatus::Unavailable)
        << ", \"tunnel_interface\": "
        << json_string(snapshot.firewall.vpn.active_tunnel_interfaces.empty()
                           ? ""
                           : snapshot.firewall.vpn.active_tunnel_interfaces.front(),
                       snapshot.firewall.vpn.interface_scan_available
                           ? ObservationStatus::Available
                           : ObservationStatus::Unavailable)
        << "}"
        << ", \"vpn_route\": {\"status\": \"unavailable\", \"uses_tunnel\": null, \"diagnostic\": "
           "\"route verification is not implemented\"}"
        << ", \"dns_path\": {\"status\": \"unavailable\", \"uses_tunnel\": null, \"diagnostic\": "
           "\"DNS path verification is not implemented\"}"
        << ", \"kill_switch\": {\"status\": \"unavailable\", \"enabled\": null, \"diagnostic\": "
           "\"kill-switch verification is not implemented\"}},\n"
    ...
    for (std::size_t index = 0; index < snapshot.evidence_components.size(); ++index) {
        ...
        output << (index == 0U ? "" : ",") << "{\"component\":\"" << escape(component.component)
               << "\",\"status\":\"" << evidence_status(component.status) << "\",\"detail\":\""
               << escape(component.detail) << "\"}";
    }
    output << "]},\n"
           << "  \"recommendation\": {\"id\": \"" << escape(recommendation.finding_id)
           << "\", \"severity\": \"" << severity(recommendation.severity) << "\", \"category\": \""
           << category(recommendation.category) << "\", \"summary\": \""
           << escape(recommendation.summary) << "\", \"destination\": \""
