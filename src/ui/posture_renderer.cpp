...
void PostureRenderer::show_dashboard_snapshot(const FirewallState &state) const {
    ...
    const std::string vpn =
        ...
        : state.vpn.active_tunnel_interfaces.empty()
            ? ui_.muted("no active VPN tunnel")
            : ui_.success("VPN: " + items_or_none(state.vpn.active_tunnel_interfaces));
    ...
    const std::string denied =
        !state.security_signals.kernel_journal_available ? ui_.warning("denied log unavailable")
    ...
    const std::string journal =
        !state.security_signals.firewalld_journal_available
            ? ui_.warning("firewalld journal unavailable")
            : std::to_string(state.security_signals.firewalld_service_events) + " firewalld events";
...
void PostureRenderer::show_zones(const FirewallState &state, const std::string &title,
    ...
    for (const auto &[name, zone] : state.runtime_zones) {
        ...
        const std::string markers =
            std::string(active ? "  (active)" : "") + (default_zone ? "  (default)" : "");
