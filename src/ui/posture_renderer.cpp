...
ExposureCounts active_exposure_counts(const FirewallState& state) {
    ...
    for (const auto& [zone_name, zone] : state.runtime_zones) {
        ...
        counts.services += zone.services.size(); counts.ports += zone.ports.size(); counts.rich_rules += zone.rich_rules.size(); counts.forward_ports += zone.forward_ports.size();
        counts.forwarding = counts.forwarding || zone.forward; counts.masquerade = counts.masquerade || zone.masquerade;
    }
    return counts;
}

std::string active_path_summary(const FirewallState& state) {
    std::string result;
    for (const auto& [zone, interfaces] : state.active_zone_interfaces) for (const auto& interface : interfaces) result += (result.empty() ? "" : ", ") + interface + " → " + zone;
...
void PostureRenderer::show_dashboard_snapshot(const FirewallState& state) const {
    ...
    ui_.key_value("Firewall", state.active ? ui_.success_badge("ACTIVE") : ui_.danger_badge("INACTIVE"));
    ui_.key_value("Policy", "zone: " + (state.default_zone.empty() ? ui_.warning("unknown") : ui_.accent(state.default_zone)) + "  •  boot: " + (state.enabled ? ui_.success("enabled") : ui_.warning("not enabled")) + "  •  config: " + (state.permanent_config_valid ? ui_.success("valid") : ui_.warning("review")));
    ui_.key_value("Mode", (state.operating_mode == OperatingMode::HostileNetwork ? ui_.warning("hostile network") : ui_.neutral_badge("NORMAL")) + "  •  denied logging: " + (state.log_denied == "off" ? ui_.muted("off") : state.log_denied.empty() ? ui_.warning("unknown") : ui_.warning(state.log_denied)));
    ui_.section("Network and exposure");
    const std::string vpn = !state.vpn.interface_scan_available ? ui_.warning("VPN scan unavailable") : state.vpn.active_tunnel_interfaces.empty() ? ui_.muted("no active VPN tunnel") : ui_.success("VPN: " + items_or_none(state.vpn.active_tunnel_interfaces));
    ui_.key_value("Path", active_path_summary(state) + "  •  " + vpn);
    if (!state.sockets.available) ui_.key_value("Exposure", ui_.warning("listener scan unavailable"));
    else {
        const auto listeners = summarize_listener_exposure(state.sockets);
        ui_.key_value("Exposure", std::to_string(listeners.logical_network_services) + " service(s)  •  " + std::to_string(listeners.network_reachable_bindings) + " bindings  •  " + std::to_string(exposure.ports) + " explicit ports  •  forwarding: " + (exposure.forwarding ? ui_.warning("enabled") : ui_.success("disabled")));
    }
    const std::string denied = !state.security_signals.kernel_journal_available ? ui_.warning("denied log unavailable") : state.security_signals.kernel_drop_or_reject_events == 0 ? ui_.success("no denied events") : ui_.warning(std::to_string(state.security_signals.kernel_drop_or_reject_events) + " denied events");
    const std::string journal = !state.security_signals.firewalld_journal_available ? ui_.warning("firewalld journal unavailable") : std::to_string(state.security_signals.firewalld_service_events) + " firewalld events";
    ui_.key_value("Telemetry (24h)", denied + "  •  " + journal);
}
void PostureRenderer::show_overview(const FirewallState& state) const {
    ...
    const auto exposure = active_exposure_counts(state);
    ui_.section("Exposure summary"); ui_.key_value("Allowed services", exposure.services == 0 ? ui_.success("NONE") : ui_.warning(std::to_string(exposure.services) + " configured")); ui_.key_value("Explicit ports", exposure.ports == 0 ? ui_.success("NONE") : ui_.warning(std::to_string(exposure.ports) + " configured")); ui_.key_value("Rich rules", std::to_string(exposure.rich_rules)); ui_.key_value("Forward ports", exposure.forward_ports == 0 ? ui_.success("NONE") : ui_.warning(std::to_string(exposure.forward_ports) + " configured")); ui_.key_value("Intra-zone forwarding", exposure.forwarding ? ui_.warning("ENABLED") : ui_.success("DISABLED")); ui_.key_value("Masquerading", exposure.masquerade ? ui_.warning("ENABLED") : ui_.success("DISABLED")); ui_.key_value("Active policies", state.active_policies.empty() ? ui_.muted("none") : items_or_none(state.active_policies));
}
...
void PostureRenderer::show_zones(const FirewallState& state, const std::string& title, ZoneView view, ZoneScope scope) const {
    ui_.section(title);
    ...
    for (const auto& [name, zone] : state.runtime_zones) {
        ...
        if (scope == ZoneScope::ActiveAndDefault && !active && !default_zone) continue;

        displayed = true;
        const std::string markers = std::string(active ? "  (active)" : "") + (default_zone ? "  (default)" : "");
        ...
        if (view == ZoneView::All || view == ZoneView::Ports) ui_.key_value("ports", annotated_port_list(zone.ports));
        ...
        if (view == ZoneView::All || view == ZoneView::Routing) {
            ...
            ui_.key_value("intra-zone forwarding", zone.forward ? ui_.warning("enabled") : ui_.success("disabled"));
        ...
        if (view == ZoneView::Drift) {
            ...
            const bool same = permanent != state.permanent_zones.end() && zone_policies_equal(zone, permanent->second);
            ui_.key_value("policy configuration", same ? ui_.success("matches permanent") : ui_.warning("differs from permanent"));
            if (permanent != state.permanent_zones.end() && zone.interfaces != permanent->second.interfaces) ui_.key_value("interface binding", ui_.muted("runtime assignment differs; commonly managed by NetworkManager"));
        }
        std::cout << '\n';
    }
    if (!displayed) std::cout << "  " << ui_.warning("No active or default zone policy was reported.") << '\n';
}
