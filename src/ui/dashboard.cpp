...
std::string menu_title(DashboardMenu menu) {
    ...
    if (menu == DashboardMenu::Network)
        return "Network and exposure";
    if (menu == DashboardMenu::Security)
        return "Security and local evidence";
    return "Main menu";
}
} // namespace
...
void OperationsDashboard::show_menu(const FirewallState &state, DashboardMenu menu) const {
    ...
    if (menu == DashboardMenu::Main) {
        ui_.heading("DEF CON FIREWALL CONTROL",
                    "READ-ONLY POSTURE CONSOLE  //  THEME: " + ui_.theme_name());
        posture_.show_dashboard_snapshot(state);
        ui_.section("Main menu");
        std::cout << "  " << ui_.keycap("1") << " Firewall policy and posture\n"
                  << "      Zones, services, ports, rules, routing, and configuration "
                     "drift\n"
...
void OperationsDashboard::show_detail_header(const std::string &title,
                                             const std::string &subtitle) const {
    ui_.clear();
    ui_.heading("DEF CON FIREWALL CONTROL",
                title + "  //  READ-ONLY" + (subtitle.empty() ? "" : "  //  " + subtitle));
}
void OperationsDashboard::show_navigation_help(DashboardMenu menu) const {
    ...
    std::cout << "  "
              ...
              << '\n'
              << "  "
              << ui_.muted("Use the Security menu for advisory, evidence, and "
...
void OperationsDashboard::pause(DashboardMenu return_menu) const {
    std::cout << '\n' << ui_.muted("Press Enter to return to " + menu_title(return_menu) + "...");
    std::string ignored;
